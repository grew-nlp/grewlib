...
clause_item:
        ...
        | feat_id1_loc=feature_ident_with_loc EQUAL rhs=ID
             { let (feat_id1,loc)=feat_id1_loc in
              ...
              | Ast.Simple value ->
                Clause_const (Ast.Feature_cmp_value (Eq,feat_id1, value), loc)
              | Ast.Pointed (s1, s2) ->
        ...
        | feat_id1_loc=feature_ident_with_loc EQUAL rhs=STRING
            { let (feat_id1,loc)=feat_id1_loc in Clause_const (Ast.Feature_cmp_value (Eq, feat_id1, rhs), loc) }

        /*   X.cat = 12.34   */
        | feat_id1_loc=feature_ident_with_loc EQUAL rhs=number
            { let (feat_id1,loc)=feat_id1_loc in Clause_const (Ast.Feature_cmp_value (Eq, feat_id1, Feature_value.of_float rhs), loc) }
        ...
        | feat_id1_loc=feature_ident_with_loc SLASH fn2=simple_id EQUAL rhs=STRING
            { let (feat_id1,loc)=feat_id1_loc in Clause_const (Ast.Feature_else (feat_id1, fn2, rhs), loc) }

        /*   X.ExtPos/upos = 22   */
        | feat_id1_loc=feature_ident_with_loc SLASH fn2=simple_id EQUAL rhs=number
            { let (feat_id1,loc)=feat_id1_loc in Clause_const (Ast.Feature_else (feat_id1, fn2, Feature_value.of_float rhs), loc) }
        ...
        | lhs_loc=simple_or_pointed_with_loc DISEQUAL rhs =simple_or_pointed
             {  match (lhs_loc,rhs) with
              | ((Ast.Pointed feat_id,loc), Ast.Simple value) ->
                Clause_const (Ast.Feature_cmp_value (Neq, feat_id, value), loc)
        ...
        | lhs_loc=simple_or_pointed_with_loc DISEQUAL rhs=STRING
            { match lhs_loc with
              | (Ast.Pointed feat_id, loc) -> Clause_const (Ast.Feature_cmp_value (Neq, feat_id, rhs), loc)
        ...
        | lhs_loc=simple_or_pointed_with_loc DISEQUAL rhs=number
            { match lhs_loc with
              | (Ast.Pointed feat_id, loc) -> Clause_const (Ast.Feature_cmp_value (Neq, feat_id, Feature_value.of_float rhs), loc)
              | (_,loc) -> Error.build ~loc "syntax error in constraint"
            }
...
command:
        ...
        | APPEND_FEATS src_loc=simple_id_with_loc ARROW tar=simple_id
            { let (src,loc) = src_loc in (Ast.Concat_feats (Append, src, tar, Regexp.all, ""), loc) }

        /*   prepend_feats M ==> N   */
        | PREPEND_FEATS src_loc=simple_id_with_loc ARROW tar=simple_id
            { let (src,loc) = src_loc in (Ast.Concat_feats (Prepend, src, tar, Regexp.all, ""), loc) }

        /*   append_feats "+" M ==> N   */
        | APPEND_FEATS sep=STRING src_loc=simple_id_with_loc ARROW tar=simple_id
            { let (src,loc) = src_loc in (Ast.Concat_feats (Append, src, tar, Regexp.all, sep), loc) }

        /*   prepend_feats "+" M ==> N   */
        | PREPEND_FEATS sep=STRING src_loc=simple_id_with_loc ARROW tar=simple_id
            { let (src,loc) = src_loc in (Ast.Concat_feats (Prepend, src, tar, Regexp.all, sep), loc) }
